...
# 合成「找格線用」的那張底圖時，先把每一份的墨跡加粗幾個像素。
#
# **為什麼要有這個**：承辦人的 C 表（房屋稅來文）是影印件，格線又細又淡。
# 逐像素取最亮的規則對它太狠 —— 每份掃描對位差一兩個像素，三份一疊就把
# 格線吃到只剩斷斷續續的殘骸，程式再也認不出「一排格子」，身分證那一欄
# 就退回整行讀，十個字讀成六碼、七碼。
#
# 拿承辦人給的 C 表實測：單張原稿找得到 5~11 條格線，三份／四份／五份
# 合成的底圖**全部找不到**；加粗 4 像素之後找到 11 條（十個格子正好要 11 條）。
#
# 加粗只用在「找格線」那張。相減用的底圖一個像素都不會動 ——
# 把印刷筆畫加粗再拿去相減，會把寫在格線上的手寫筆畫一起削掉，
# 那是 subtract() 的註解裡就寫著的坑。
GRID_THICKEN = 4


def compose(samples, reference=None, thicken=0):
    """把多份同款掃描件合成一張只剩印刷版面的底圖。

    samples    同一種表格、同一個版本的灰階掃描件
    reference  以哪一份的座標系為準；省略就用第一份
    thicken    合成前先把每一份的墨跡加粗幾個像素（見 GRID_THICKEN）。
               0 就是原本的行為，相減用的底圖一律用 0。
    ...
    if thicken:
        # 灰階 erode 就是「讓暗的長胖」。印刷線每一份都在同一個位置，
        # 加粗之後吃得下對位誤差；手寫每一份位置完全不同，加粗一兩個
        # 像素照樣被別份的白吃掉。
        kernel = np.ones((thicken, thicken), np.uint8)
        stack = [cv2.erode(item, kernel) for item in stack]
