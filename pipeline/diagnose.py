...
# 簡體的「号」「楼」「邻」也是結構字，要跟繁體一樣留著。
#
# 2026-09-24 承辦人問「樓讀不出來是怎樣?」「樓又錯了是怎樣??」——
# 那兩件其實讀到了「号」「楼」，程式也照簡繁對照表換回來了，
# 但報告把它們遮成「字」，看起來就像整個沒讀到。**報告騙了人。**
# 遮罩的目的是拿掉個資，而「号」跟「號」一樣是表格的格式，不是誰的資料。
KEEP = set("路街道段巷弄號樓層之縣市區里鄰村鄉鎮地建号楼邻")
